#!/usr/bin/env python3
"""
Test Inference and Persistence Script

This script focuses only on testing inference capabilities and model persistence
after training is complete. It loads a pre-trained model and tests real inference.
"""
...
def test_real_inference(generator):
    ...
    print("🎭 Testing real inference capabilities...")
    ...
    try:
        ...
        for i, test_case in enumerate(test_cases):
            ...
            print(f"\n🧪 Test {i+1}/{total_tests}: {description}")
            ...
            try:
                ...
                for temp in temperatures:
                    for max_len in max_lengths:
                        try:
                            print(f"   🔧 Testing T={temp}, max_len={max_len}...")
                            ...
                            if isinstance(response, tuple):
                                ...
                                print(f"   ✅ Response: \"{response_text}\" (confidence: {confidence:.3f})")
                            else:
                            ...
                            print(f"   ❌ Failed with T={temp}, max_len={max_len}: {param_error}")
                            continue
                    else:
                        continue  # No success with any max_length for this temperature
                    break  # Success with this temperature, move to next test case
                else:
                    print(f"   ❌ All parameter combinations failed for this prompt")
                    
            except Exception as e:
                print(f"   ❌ Test failed: {e}")
                print(f"   🔍 Error type: {type(e).__name__}")
                if "embeddings" in str(e).lower():
                    print("   🔍 This appears to be an embeddings-related error")
                elif "tokenizer" in str(e).lower():
                    print("   🔍 This appears to be a tokenizer-related error")
                elif "response_generator" in str(e).lower():
                    print("   🔍 This appears to be a response generator error")
        
        print(f"\n📊 Test Results: {successful_tests}/{total_tests * 3} parameter combinations successful")
        ...
        try:
            if hasattr(generator, 'system_message_support') and generator.system_message_support:
                ...
                print(f"   ✅ System message response: \"{response}\"")
            else:
    ...
    try:
        ...
        try:
            ...
            if embedder is not None:
                ...
                print("⚠️ No embedder found")
        except Exception as embed_error:
            print(f"❌ Embedding test failed: {embed_error}")
        
        return True
        
    except Exception as e:
        print(f"❌ Tokenizer/embedder integration test failed: {e}")
        traceback.print_exc()
        return False
...
def test_response_generator_directly(generator):
    ...
    print("🎭 Testing response generator directly...")
    
    try:
        # Access the response generator
        response_generator = getattr(generator, '_response_generator', None)
        if response_generator is None:
            print("❌ No response generator found")
            return False
        
        print(f"✅ Response generator: {type(response_generator).__name__}")
        
        # Test direct response generation
        test_input = "Hello, how are you?"
        print(f"🧪 Testing direct response generation: '{test_input}'")
        ...
        try:
            ...
            print(f"❌ Direct response generation failed: {gen_error}")
            traceback.print_exc()
            return False
...
def main():
    ...
    print("🚀 LSM Inference and Persistence Test")
    print("=" * 60)
    ...
    if passed == total:
        print("🎉 All tests passed! Inference and persistence are working correctly.")
    else:
